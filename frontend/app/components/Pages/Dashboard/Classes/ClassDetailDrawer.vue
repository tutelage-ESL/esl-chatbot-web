...
const tutors = computed(() => props.cls?.members?.filter(m => m.role === 'TUTOR' || m.role === 'ADMIN') ?? [])
...
<template>
  <UiSheet :open="open" @update:open="emit('update:open', $event)">
    <UiSheetContent
      side="right"
      class="w-full sm:max-w-105 p-0 gap-0 flex flex-col"
      ...
      <template v-else>
        ...
        <UiSheetHeader class="p-5 pb-4 shrink-0" style="border-bottom:1px solid var(--border-inner)">
          <div class="flex items-start gap-3 pr-6">
            <div class="size-11 rounded-xl flex items-center justify-center shrink-0 mt-0.5" style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.2)">
              <AppIconsax name="BookSaved" color="var(--color-brand-primary)" :size="18" />
            </div>
            <div class="flex-1 min-w-0">
              <UiSheetTitle class="text-[16px] font-semibold leading-snug truncate pr-2" :style="`color:var(--text-heading)`">{{ cls.className }}</UiSheetTitle>
              <AppText v-if="cls.classCategory" size="12" color="neutral-400" class-list="block mt-0.5" :style="`color:var(--text-muted)`">{{ cls.classCategory }}</AppText>
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="rounded-xl p-3" style="background:var(--surface-raised);border:1px solid var(--border-inner)">
              <div class="flex items-center gap-2 mb-1">
                ...
                <AppText size="10" weight="semibold" color="neutral-400" :uppercase="true" class-list="tracking-[0.12em]" :style="`color:var(--text-subtle)`">Members</AppText>
              </div>
              <AppText size="20" weight="semibold" color="black" :style="`color:var(--text-heading)`">{{ cls.members.length }}</AppText>
            </div>
            <div class="rounded-xl p-3" style="background:var(--surface-raised);border:1px solid var(--border-inner)">
              <div class="flex items-center gap-2 mb-1">
                <AppIconsax name="Calendar" color="var(--color-text-subtle)" :size="12" />
                <AppText size="10" weight="semibold" color="neutral-400" :uppercase="true" class-list="tracking-[0.12em]" :style="`color:var(--text-subtle)`">Created</AppText>
              </div>
              <AppText size="13" weight="semibold" color="black" :style="`color:var(--text-heading)`">{{ fmtDate(cls.createdAt) }}</AppText>
            </div>
          </div>
          ...
          <div v-if="isTutorOrAdmin" class="rounded-xl overflow-hidden" style="border:1px solid var(--border-card)">
            <div class="px-4 py-3 flex items-center justify-between" style="background:var(--surface-raised);border-bottom:1px solid var(--border-inner)">
              <div class="flex items-center gap-2">
                <AppIconsax name="Key" color="var(--color-text-subtle)" :size="13" />
                <AppText size="11" weight="semibold" color="neutral-400" :uppercase="true" class-list="tracking-[0.12em]" :style="`color:var(--text-subtle)`">Class code</AppText>
              </div>
              <div class="flex items-center gap-1.5">
                <AppIconsax :name="statusConfig.icon" :size="11" :style="statusConfig.style" />
                <AppText size="11" color="neutral-400" :style="statusConfig.style">{{ statusConfig.label }}</AppText>
              </div>
            </div>
            <div class="p-4 space-y-3" style="background:var(--surface-card)">
              <div class="flex items-center px-4 py-3 rounded-xl" style="background:var(--surface-raised);border:1px solid var(--border-inner)">
                <AppText size="18" weight="bold" font-family="mono" class-list="flex-1 tracking-[0.35em] select-all text-center" :style="`color:var(--text-heading)`">
                  {{ cls.classCode }}
                </AppText>
              </div>
              <div class="flex items-center gap-2">
          ...
          <!-- Admin CRUD actions -->
          <div v-if="isAdmin" class="flex items-center gap-2" style="padding-top:4px">
            <AppButton
              variant="secondary"
              size="36"
              radius="8"
            ...
            <AppButton
              ...
              radius="8"
              icon="Trash"
              :icon-config="{ color: '#ef4444', size: 14 }"
              text="Delete"
              class="flex-1"
          ...
          <!-- Members: tutors/admins -->
          <div v-if="tutors.length > 0">
            <AppText size="10" weight="semibold" color="neutral-400" :uppercase="true" class-list="tracking-[0.18em] block mb-3" :style="`color:var(--text-subtle)`">
            ...
            <div class="space-y-0.5">
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  <AppText size="13" weight="medium" color="black" class-list="truncate block" :style="`color:var(--text-heading)`">
                    {{ m.user.displayName || m.user.username }}
                  </AppText>
                  <AppText size="11" color="neutral-400" :style="`color:var(--text-muted)`">@{{ m.user.username }}</AppText>
                </div>
          ...
          <div v-if="students.length > 0">
            ...
            <div class="space-y-0.5">
              ...
              >
                <UiAvatar class="size-9 shrink-0">
                  <UiAvatarImage v-if="m.user.avatarUrl" :src="m.user.avatarUrl" :alt="m.user.displayName" />
                  <UiAvatarFallback class="text-[12px] font-semibold font-poppins" style="background:var(--surface-well);color:var(--text-muted)">
                    {{ avatarInitial(m.user.displayName || m.user.username) }}
                  </UiAvatarFallback>
                </UiAvatar>
                <div class="flex-1 min-w-0">
                ...
                <span class="text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full font-poppins shrink-0" style="background:var(--surface-well);color:var(--text-muted)">
                  STUDENT
                </span>
              </div>
            </div>
          ...
          <div v-if="!cls.members?.length" class="text-center py-8">
            ...
            <AppText size="13" color="neutral-400" class-list="block mt-2" :style="`color:var(--text-muted)`">No members yet</AppText>
          </div>
