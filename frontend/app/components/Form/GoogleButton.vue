    ...
    googleSdkLoaded((google) => {
        ...
        google.accounts.id.prompt((notification: any) => {
            if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                ...
                document.getElementById('google-signin-fallback')?.querySelector('div[role=button]')?.dispatchEvent(new MouseEvent('click'))
                isLoading.value = false
            }
        })
    })
...
<template>
    <div>
        ...
        <!-- Hidden fallback container for Google's rendered button -->
        <div id="google-signin-fallback" class="hidden" />
    </div>
</template>
