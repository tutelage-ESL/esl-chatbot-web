...
<template>
  <header
    ...
    <div class="hidden md:flex items-center gap-2 flex-1 max-w-md mx-8">
      <button
        ...
        <kbd
          class="ml-auto font-mono text-[10px] px-1.5 py-0.5 rounded bg-white dark:bg-black/30 border border-black/5 dark:border-white/10">
          ⌘K
        </kbd>
      </button>
    </div>
  ...
  <Teleport to="body">
    <Transition name="cmd-fade">
      <div v-if="cmdOpen" class="fixed inset-0 z-50 flex items-start justify-center pt-[15vh] px-4"
        @click.self="closeCmd">
        ...
        <div
          ...
          <div class="flex items-center gap-3 px-4 py-3.5 border-b border-black/6 dark:border-white/6">
            ...
            <input ref="cmdInput" v-model="cmdQuery" type="text" placeholder="Search pages, lessons, words…"
              class="flex-1 bg-transparent text-[13.5px] text-brand-ink dark:text-white placeholder:text-zinc-400 outline-none" />
            <kbd
              class="font-mono text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-white/6 border border-black/5 dark:border-white/10 text-zinc-400 shrink-0">
              Esc
            </kbd>
          </div>
          ...
          <div class="py-2 max-h-72 overflow-y-auto">
            ...
            <template v-else>
              ...
              <button v-for="action in filteredActions" :key="action.to"
                class="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-zinc-50 dark:hover:bg-white/4 transition group"
                :class="route.path === action.to ? 'bg-brand-primary/5' : ''" @click="selectAction(action.to)">
                <div class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition" :class="route.path === action.to
                ...
                <AppIconsax name="ArrowRight" color="currentColor" :size="11"
          ...
          <div
            class="px-4 py-2.5 border-t border-black/6 dark:border-white/6 flex items-center gap-3 text-[10.5px] text-zinc-400">
            <span><kbd
                class="font-mono px-1 py-0.5 rounded bg-zinc-100 dark:bg-white/6 border border-black/5 dark:border-white/10 text-[10px]">↵</kbd>
              select</span>
            <span><kbd
                class="font-mono px-1 py-0.5 rounded bg-zinc-100 dark:bg-white/6 border border-black/5 dark:border-white/10 text-[10px]">Esc</kbd>
              close</span>
            <span class="ml-auto"><kbd
                class="font-mono px-1 py-0.5 rounded bg-zinc-100 dark:bg-white/6 border border-black/5 dark:border-white/10 text-[10px]">⌘K</kbd>
              toggle</span>
          </div>
        </div>
      </div>
